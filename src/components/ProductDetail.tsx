...
export function ProductDetail({ product }: { product: Product }) {
  ...
  return (
    <div className="mx-auto grid max-w-6xl gap-10 px-4 py-10 sm:px-6 lg:grid-cols-2 lg:gap-14 lg:px-8 lg:py-16">
      <div className="relative aspect-square overflow-hidden rounded-[2rem] scene-blush shadow-[var(--shadow-card)]">
        <div className="ring-3d absolute left-[8%] top-[10%] h-[38%] w-[38%] rounded-full" />
        <div className="podium-blush absolute bottom-[12%] left-1/2 h-[10%] w-[56%] -translate-x-1/2 rounded-[999px]" />
      ...
      <div>
        ...
        <button
          ...
          className="mt-8 min-h-12 w-full rounded-md bg-bordeaux px-8 py-4 text-[12px] font-semibold tracking-[0.16em] text-white uppercase transition hover:bg-bordeaux-deep active:scale-[0.99] sm:w-auto"
        >
        ...
        <ul className="mt-8 grid gap-3 border-t border-line pt-6 sm:grid-cols-2">
          {reassurance.map((item) => (
            <li key={item.id} className="text-[13px] text-charcoal-soft">
              <strong className="block text-charcoal">{item.line1}</strong>
              {item.line2}
            </li>
          ))}
        </ul>
